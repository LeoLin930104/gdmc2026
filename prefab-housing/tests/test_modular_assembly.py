...
def test_block_generation_stages_expose_construction_order() -> None:
    ...
    assert stage_names.index("populate_interiors") < stage_names.index("entry_door_module")
    assert stage_names.index("entry_door_module") < stage_names.index("interior_site_footprint_clip")

    exterior_stages = tuple(
        stage
        for stage in result.block_stages
        if stage.name not in {"populate_interiors", "entry_door_module", "interior_site_footprint_clip"}
    )
