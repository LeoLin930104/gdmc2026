"""One-shot orchestrator: identity -> bake -> town -> narrative -> lighting.
...
narrative layer before the final lighting audit.

Launch this through uv so every step uses the workspace-managed environment.
Minecraft + the GDMC HTTP interface must be running.

    uv run python run_settlement.py
    uv run python run_settlement.py --dry-run        # print the steps only
"""
...
from wallface_narrative import biome_family, package_signature
...
def _needs_bake(mood: str, family: str, biome: str) -> bool:
    ...
    for seed in BASE_SEEDS:
        ...
        if not (CACHE_DIR / pkg).exists():
            return True
        if stamps.get(pkg) != package_signature(mood, biome, seed):
            return True
    return False
...
def main() -> None:
    ...
    _run([*UV_PYTHON, str(RUN_NARRATIVE), "--identity-only", *theme_args, *biome_args],
         label="1/5 identity (biome + mood)", dry_run=args.dry_run)
    ...
    if args.skip_bake:
        print("[run_settlement] 2/5 bake skipped (--skip-bake).")
    elif not args.rebake and not _needs_bake(mood, family, biome):
        print(f"[run_settlement] 2/5 bake skipped; variants for mood={mood!r} family={family!r} are up to date.")
    else:
        _run([*UV_PYTHON, str(BAKE), "--biome", biome, "--moods", mood],
             label=f"2/5 bake wallpaper variants (mood={mood}, family={family})", dry_run=args.dry_run)

    # 3. Town generation: places the mood/biome-matched prefabs. Lighting is
    # deferred until after narrative, because premades/crop fields/chests can
    # create or occlude spawnable surfaces.
    _run([*UV_PYTHON, str(TOWN), "--no-town-lighting"],
         label="3/5 town generation", dry_run=args.dry_run)

    # 4. Narrative layer: datapack + premades, reusing the identity.
    _run([*UV_PYTHON, str(RUN_NARRATIVE), *theme_args],
         label="4/5 narrative layer", dry_run=args.dry_run)

    # 5. Final lighting must be the last world-writing pass.
    _run([*UV_PYTHON, str(TOWN), "--lighting-only"],
         label="5/5 final lighting reverse sweep", dry_run=args.dry_run)
